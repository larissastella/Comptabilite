...
export default function InvoiceDetail() {
  ...
  const markPaid = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['invoice', id] }); toast.success('Marquée comme payée'); },
  ...
  const sendInvoice = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['invoice', id] }); toast.success('Facture envoyée'); },
    onError: (err: Error) => toast.error(err.message),
  });
  ...
  return (
    <div className="p-4 sm:p-6 max-w-4xl mx-auto">
      ...
      <div id="invoice-print" className="bg-white rounded-2xl border border-gray-100 p-8 print:rounded-none print:border-0 print:shadow-none">
        ...
        {invoice.customers && (
          <div className="mb-8">
            <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Facturé à</h3>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b-2 border-[#0F2A3D]">
                <th className="text-left py-2 text-sm font-semibold text-[#0F2A3D]">Description</th>
                <th className="text-right py-2 text-sm font-semibold text-[#0F2A3D]">Qté</th>
                <th className="text-right py-2 text-sm font-semibold text-[#0F2A3D]">Prix unit.</th>
                <th className="text-right py-2 text-sm font-semibold text-[#0F2A3D]">Rem %</th>
                <th className="text-right py-2 text-sm font-semibold text-[#0F2A3D]">TVA %</th>
                <th className="text-right py-2 text-sm font-semibold text-[#0F2A3D]">Total HT</th>
                <th className="text-right py-2 text-sm font-semibold text-[#0F2A3D]">Total TTC</th>
              </tr>
            </thead>
            <tbody>
        ...
        <div className="flex justify-between items-end">
          <div className="max-w-xs">
            ...
            {(tenant?.bank_details as Record<string, string>)?.iban && (
              <div className="mt-4">
                <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Coordonnées bancaires</p>
        ...
        {tenant?.cachet_url && (
          <div className="mt-8 flex justify-end">
            <div className="text-center">
              <p className="text-xs text-gray-400 mb-1">Cachet officiel</p>

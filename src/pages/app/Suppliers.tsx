    ...
    <div className="p-4 sm:p-6 dark:bg-surface-0">
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <button onClick={openCreate} className="px-4 py-2 bg-[#10B981] text-white text-sm rounded-xl hover:bg-[#0d9e6e]">{t('suppliers.new')}</button>
        </div>
      ) : (
        <>
          {/* Desktop table */}
                  ...
                  <tr key={s.id} className="hover:bg-gray-50 group">
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => openEdit(s)} className="p-1.5 text-gray-400 hover:text-[#10B981] rounded-lg hover:bg-gray-100"><Edit2 className="w-3.5 h-3.5" /></button>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-0 sm:p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-5 sm:p-6 sm:my-8 min-h-screen sm:min-h-0 rounded-none sm:rounded-2xl">
            <h2 className="text-lg font-bold text-gray-900 mb-5">{editSupplier ? 'Modifier fournisseur' : t('suppliers.new')}</h2>

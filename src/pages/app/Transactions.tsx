...
export default function Transactions() {
  ...
  const save = useMutation({
    mutationFn: async () => {
      if (!isBalanced) throw new Error('La saisie doit être équilibrée (débit = crédit)');
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  {['Date', 'Référence', 'Description', 'Type', 'Statut', ''].map(h => (
              ...
              <tbody className="divide-y divide-gray-50">
                {filtered.map(tx => (
                  <tr key={tx.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3.5">
                      {tx.is_posted
                        ? <Badge variant="success">Comptabilisé</Badge>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-start justify-center overflow-y-auto p-0 sm:p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl sm:my-8 min-h-screen sm:min-h-0 rounded-none sm:rounded-2xl">
            <div className="flex items-center justify-between px-5 sm:px-6 py-4 border-b border-gray-100">
              <h2 className="text-lg sm:text-xl font-bold text-gray-900">Nouvelle écriture comptable</h2>
            ...
            <div className="p-5 sm:p-6 space-y-5">
              ...
              <div className="hidden sm:block border border-gray-200 rounded-xl overflow-hidden">
                ...
                <div className="grid grid-cols-12 px-4 py-3 bg-gray-50 border-t border-gray-200 gap-2 text-sm font-semibold">
                  <div className="col-span-8 text-gray-600">Total</div>
              ...
              {!isBalanced && (
                <p className="text-xs text-red-500">⚠ La saisie n'est pas équilibrée — différence: {formatCurrency(Math.abs(totalDebit - totalCredit))}</p>
              )}
